...
import { FORGOT_PASSWORD_MUTATION } from "@/queries/auth/forgotUserPassword";
...
export function useForgotPassword() {
  ...
  const onSubmit = async (data: ForgotPasswordSchema) => {
    if (isCooldown) {
      notify.warning("Aguarde um momento", `Você pode tentar novamente em ${counter} segundos.`);
      return;
    }
    ...
    try {
      ...
      const token = result?.forgotUserPassword?.token;
      if (!token) {
